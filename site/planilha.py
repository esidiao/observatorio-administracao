...
Uso (normalmente chamado por site/build.py):
    python site/planilha.py --saida site/dist/dados/observatorio-administracao.xlsx
...
def aba_municipios(wb, dir_municipios, nomes_uf):
    ...
    campos = ["vagas_presencial", "cursos_presencial", "n_ies", "matriculas",
              ...
              "administradores_rais", "empresas_atuantes",
              "pessoal_ocupado"]
...
def aba_leia_me(wb, nacional, proveniencia):
    ...
    linhas = [
        ("Observatório Nacional da Formação em Administração", None),
        ("", None),
        ("Autoria", "Edson Sidião de Souza Júnior — sidiao@i9educar.com"),
        ("Site", "https://esidiao.github.io/observatorio-administracao/"),
    ...
    rotulos = {"censo": "Censo da Educação Superior (INEP)",
               ...
               "rais": "RAIS (Ministério do Trabalho e Emprego)",
               "cempre": "CEMPRE — Cadastro Central de Empresas (IBGE)",
...
def gerar(saida, dir_dados=None, nomes_uf=None):
    ...
    wb.properties.title = "Observatório Nacional da Formação em Administração"
    wb.properties.creator = "Edson Sidião de Souza Júnior"
    wb.properties.description = (
        "Indicadores de acesso territorial, qualidade e absorção profissional "
...
def main():
    ...
    p.add_argument("--saida", default=str(
        RAIZ / "site" / "dist" / "dados" / "observatorio-administracao.xlsx"))
