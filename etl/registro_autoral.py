...
fora os dados públicos (que são do INEP, do Ministério do Trabalho e do IBGE,
não deste projeto) e as bibliotecas de terceiros em `site/static/vendor` e
...
AUTOR = {
    ...
    "obra": "Observatório Nacional da Formação em Administração",
    "descricao": (
        "Sítio estático data-driven com indicadores de acesso territorial, "
        "qualidade e absorção profissional dos cursos de Administração no "
        "Brasil: formulação e cálculo dos índices ICT, E, IAF e IAP e da "
        "densidade de administradores por mil ocupados, catálogo de "
        "indicadores, textos e desenho editorial."
    ),
}
