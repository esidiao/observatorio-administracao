...
Entrada:  etl/dados/curso_administracao_<ano>.csv  (de extrair_censo.py)
...
Em Administração isso não é detalhe: DOIS TERÇOS da capacidade do país são EaD.
Medido no Censo 2024, sobre os 19.205 registros do rótulo exato, as linhas de
sede carregam 752.523 vagas contra 372.698 das presenciais — 66,9% do total.
Somar `QT_VG_TOTAL` por `SG_UF` sem separar as camadas devolve as vagas EaD
como zero, e o número resultante parece plausível: seria "372.698 vagas de
Administração no Brasil", que é um terço da verdade.
...
a linha de sede vem com `SG_UF` em branco. E isso É uma limitação, não uma
solução: a vaga fica registrada onde está a mantenedora, não onde está o aluno.
Toda página que mostra vagas EaD precisa dizer isso.
...
Farmácia a diferença em SP foi de 0,06 para 0,36; no de Fonoaudiologia, de
0,07 para 0,18. Aqui, com 67% da capacidade em EaD, a diferença é maior ainda.
"""
import argparse
import csv
import json
...
def carregar_cursos(ano, slug="administracao"):
    caminho = DADOS / f"curso_{slug}_{ano}.csv"
    if not caminho.exists():
        raise SystemExit(f"[ERRO] {caminho} ausente. Rode etl/extrair_censo.py antes.")
    with open(caminho, encoding="utf-8") as f:
...
def main():
    ...
    p.add_argument("--slug", default="administracao",
                   help="slug do recorte gravado por extrair_censo.py")
    ...
    linhas = carregar_cursos(args.ano, args.slug)
