...
Uso:
    ...
    python etl/extrair_censo.py --ano 2024 --rotulo "Administração"
    python etl/extrair_censo.py --ano 2024 --listar-rotulos administra
...
Para Administração a diferença é grande e silenciosa. Medido no Censo 2024:
o rótulo exato tem 19.205 registros; buscar por substring "administra"
acrescenta 4.905 registros de outros três rótulos —

    3.530  'Administração pública'
    1.367  'Programas interdisciplinares abrangendo negócios, administração e
            direito'
        8  'Programas abrangendo negócios, administração e direito em processo
            de definição da classificação'

— 25% a mais, sem erro nenhum na tela. Administração pública é decisão de
escopo declarada, não acidente de busca: ela FICA DE FORA deste observatório,
é área de avaliação separada no CPC 2022 e tem página própria explicando isso.
...
def extrair(ano, rotulo):
    ...
    # 4 letras. Para "Administração" o fragmento vira "ADMI", que casa com os
    # três vizinhos conhecidos — inclusive 'Programas interdisciplinares
    # abrangendo negócios, administração e direito', onde a palavra nem é a
    # primeira. Um aviso que não dispara no caso conhecido não vale nada.
    ...
    (DADOS / f"_censo_{ano}.json").write_text(
        json.dumps(prov, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n")
...
def main():
    ...
    p.add_argument("--rotulo", default="Administração",
                   help="rótulo CINE EXATO (default: Administração)")
